...
<xsl:stylesheet 
	...
	<xsl:template match="/">
		<oai_dc:dc xmlns:oai_dc="http://www.openarchives.org/OAI/2.0/oai_dc/" 
			...
			<!-- For openAire, give dc.date embargo= embargoEnd, if the item has an embargo set via others.openAireAccess= embargoedAccess (embargoEnd was set in embargo_selector.xsl, from xoai/Utils.ItemUtils.java) -->
		...
		<!--	<xsl:for-each select="doc:metadata/doc:element[@name='created']/doc:element[@name='identifier']/doc:element[@name='doi']/doc:element/doc:field[@name='value']">  -->
		<xsl:for-each select="doc:metadata/doc:element[@name='local']/doc:element[@name='identifier']/doc:element[@name='doi']/doc:element/doc:field[@name='value']">
			...
			<!-- dc.identifier -->
		<!--	<xsl:for-each select="doc:metadata/doc:element[@name='created']/doc:element[@name='identifier']/doc:element/doc:field[@name='value']">  -->
			<xsl:for-each select="doc:metadata/doc:element[@name='local']/doc:element[@name='identifier']/doc:element/doc:field[@name='value']">
				<dc:identifier><xsl:value-of select="." /></dc:identifier>
			</xsl:for-each>
			<!-- dc.identifier.* other than doi-->
			<!--  <xsl:for-each select="doc:metadata/doc:element[@name='created']/doc:element[@name='identifier']/doc:element/doc:element/doc:field[@name='value'][not(contains(text(), 'doi.org'))]">   -->
			<xsl:for-each select="doc:metadata/doc:element[@name='local']/doc:element[@name='identifier']/doc:element/doc:element/doc:field[@name='value'][not(contains(text(), 'doi.org'))]">
				<dc:identifier><xsl:value-of select="." /></dc:identifier>
			</xsl:for-each>
			<!-- dc.language -->
			...
			<xsl:if test="not(doc:metadata/doc:element[@name='dc']/doc:element[@name='publisher']/doc:element/doc:field[@name='value'])">
				<dc:publisher><xsl:text>Friedrich-Alexander-Universitaet Erlangen-Nuernberg (FAU)</xsl:text></dc:publisher>
			</xsl:if>			
