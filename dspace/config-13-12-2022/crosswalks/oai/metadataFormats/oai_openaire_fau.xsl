...
      		    	</datacite:contributorName>
         		<datacite:contributorType>
         			<xsl:text>RelatedPerson</xsl:text>
         		</datacite:contributorType>
      		    </datacite:contributor> 	
            	    </xsl:for-each>
            	    <!-- editor => Editor -->
...
      		    </datacite:contributor> 	
                    ...
                    <!-- My Repository name as hosting organization-->
			...
			<xsl:otherwise>
				 European Commission	
	     		    	...
	     		    	<oaire:awardNumber awardURI="http://cordis.europa.eu/project/rcn/194062_en.html">
			 	  <xsl:value-of select="."/>
			 	</oaire:awardNumber>
	       	 </oaire:fundingReference>
 			</oaire:fundingReferences>
            ...
            <!-- datacite:dates and embargo  => by default, dont send info abt embargoed and private items -->
            <xsl:apply-templates
            ...
            <!-- datacite:identifier 
            <xsl:apply-templates   select="doc:metadata/doc:element[@name='dc']/doc:element[@name='identifier']/doc:element[@name='uri']"
                mode="datacite"/>  -->
                <!-- DOI saved in dc identifier doi -->
		...
		<!-- DOI saved in dc.identifier.uri when new in DSpace -->
		...
		<!-- URL as handle in DSpace -->
		<xsl:for-each select="doc:metadata/doc:element[@name='dc']/doc:element[@name='identifier']/doc:element[@name='uri']/doc:element/doc:field[@name='value'][not(contains(text(), 'doi.org'))]">
   ...
   <!-- https://openaire-guidelines-for-literature-repository-managers.readthedocs.io/en/v4.0.0/field_resourceidentifier.html -->
    ...
    <!-- datacite:identifier for dc.identifier.doi -->
    <xsl:template match="doc:element[@name='dc']/doc:element[@name='identifier']/doc:element[@name='doi']"
        mode="datacite">
        <xsl:variable name="identifierType">
            <xsl:call-template name="resolveFieldType">
                <xsl:with-param name="field" select="./doc:element/doc:field[@name='value']"/>
            </xsl:call-template>
        </xsl:variable>
        <datacite:identifier>
            <xsl:attribute name="identifierType">
     ...
     <!-- datacite:identifier for dc.identifier.urn -->
    <xsl:template match="doc:element[@name='dc']/doc:element[@name='identifier']/doc:element[@name='urn']"
        mode="datacite">
        <xsl:variable name="identifierType">
            <xsl:call-template name="resolveFieldType">
                <xsl:with-param name="field" select="./doc:element/doc:field[@name='value']"/>
   ...
   <!-- to retrieve a string in lowercase -->
    ...
    <!-- it will verify if a given field is a URN -->
    <xsl:template name="isURN">
        <xsl:param name="field"/>
        <xsl:choose>
            <xsl:when test="$field[contains(text(),'urn:nbn')]">
                <xsl:value-of select="true()"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="false()"/>
    ...
    <xsl:template name="resolveResourceTypeGeneral">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$lc_dc_type = 'bookpart'">
                <xsl:text>literature</xsl:text>
            </xsl:when>
    ...
    <xsl:template name="resolveResourceTypeURI">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$lc_dc_type = 'workingpaper'">
                <xsl:text>http://purl.org/coar/resource_type/c_8042</xsl:text>
            </xsl:when>
            <xsl:when test="$lc_dc_type = 'studythesis'">
